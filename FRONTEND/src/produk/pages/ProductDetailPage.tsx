...
const ProductDetailPage: React.FC<ProductDetailPageProps> = ({ isAuthenticated }) => {
    ...
    const renderStars = (rating: number) => {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(<IconStarFilled key={i} className="text-yellow-400" size={16} />);
        }

        if (hasHalfStar) {
            stars.push(<IconStarFilled key="half" className="text-yellow-400" size={16} />);
        }

        const remainingStars = 5 - Math.ceil(rating);
        for (let i = 0; i < remainingStars; i++) {
            stars.push(<IconStar key={`empty-${i}`} className="text-gray-300" size={16} />);
        }
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center text-sm text-gray-500 mb-6">
                <Link to="/" className="hover:text-[#0260FD99]">Home</Link>
                <span className="mx-2">/</span>
                <Link to="/products" className="hover:text-[#0260FD99]">Produk</Link>
                <span className="mx-2">/</span>
                <span className="text-gray-800">{product.name}</span>
            </div>

            <Link to="/products" className="inline-flex items-center text-[#0260FD99] hover:text-blue-700 transition-colors duration-300 mb-8">
                <IconArrowLeft className="mr-2" size={20} /> Kembali ke Katalog
            </Link>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-200">
                            <div className="text-center">
                                <IconTruck className="mx-auto mb-2 text-blue-500" size={24} />
                                <p className="text-xs text-gray-600">Gratis Ongkir</p>
                            </div>
                            <div className="text-center">
                                <IconShield className="mx-auto mb-2 text-blue-500" size={24} />
                                <p className="text-xs text-gray-600">Garansi Resmi</p>
                            </div>
                            <div className="text-center">
                                <IconRefresh className="mx-auto mb-2 text-blue-500" size={24} />
                                ...
                                <div className="space-y-4">
                                    {[1, 2, 3].map((review) => (
                                        <div key={review} className="border-b border-gray-200 pb-4">
                                            ...
                                            <p className="text-gray-700">Produk sangat bagus, sesuai dengan deskripsi. Pengiriman cepat dan packaging aman.</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
